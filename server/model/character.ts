...
import { CharacterDB, characterIdObject, usersIdObject } from "../types";
...
export class CharacterModel {
  ...
  static async postCharacterById(body: { obj: CharacterDB; userId: string }) {
    const db = await run();
    const collection = db?.collection("characters");
    const {
      obj: { mal_id },
    ...
    if (characterIsInDB === null) {
      ...
      const usersIdValue: Array<characterIdObject> = await collection
        ?.findOne({ mal_id })
  ...
  static async deleteCharacterById(mal_id: string, userId: string) {
    const db = await run();
    const collection = db?.collection("characters");
